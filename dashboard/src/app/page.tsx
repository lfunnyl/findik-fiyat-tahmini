...
export default function Home() {
  ...
  const translateMonth = (ayAdi: string) => lang === 'en' ? (monthNamesEn[ayAdi] || ayAdi) : ayAdi
    ...
    <div className="layout">
      ...
      <aside className="sidebar">
        <div style={{ marginBottom: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <div style={{ fontSize: '1.3rem', fontWeight: 700 }}>{t.sidebarTitle}</div>
            <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: 2 }}>{t.sidebarSubtitle}</div>
          </div>
          <button 
        ...
        <div style={{ marginBottom: 24 }}>
          <div className="metric-label" style={{ marginBottom: 8 }}>{t.usdRate}</div>
        ...
        <div style={{ marginBottom: 24 }}>
          <div className="metric-label" style={{ marginBottom: 8 }}>{t.monthlyRateIncrease}</div>
        ...
        <div style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', lineHeight: 1.7 }}>
          <div><strong>{t.algorithm}:</strong> {pred?.model_info?.algorithm ?? info?.model ?? 'Weighted Ensemble'}</div>
          ...
          <div style={{ marginTop: 8 }}><strong>{t.marginOfError}:</strong> %{pred?.model_info?.test_mape?.toFixed(2) ?? '5.06'}</div>
          <div><strong>Test R²:</strong> {pred?.model_info?.test_r2?.toFixed(3) ?? '0.800'}</div>
          <div style={{ marginTop: 8 }}><strong>{t.target}:</strong> {t.realUsd}</div>
          <div><strong>{t.data}:</strong> {info ? `${info.data_rows} ${t.months}` : `152 ${t.months}`}</div>
          {info?.data_end && <div><strong>{t.lastData}:</strong> {info.data_end.slice(0, 7)}</div>}
        </div>
      </aside>
      ...
      <main className="main-content">
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16 }} className="mobile-lang-toggle">
          <button 
            onClick={() => setLang(lang === 'tr' ? 'en' : 'tr')}
            style={{
              background: 'rgba(255,255,255,0.05)',
        ...
        <div className="hero fade-up">
          <div style={{ display: 'flex', gap: 10, marginBottom: 14, flexWrap: 'wrap' }}>
            ...
            <span className="badge badge-orange">{t.forecast2026}</span>
          </div>
          <h1 className="hero-title gradient-text">{t.heroTitle}</h1>
        ...
        <div className="tab-nav">
          {(['tahmin', 'model', 'whatif'] as const).map(tabKey => (
            <button key={tabKey} className={`tab-btn ${tab === tabKey ? 'active' : ''}`} onClick={() => setTab(tabKey)}>
              {tabKey === 'tahmin' ? t.tabMain : tabKey === 'model' ? t.tabModel : t.tabWhatIf}
            </button>
          ))}
        </div>
          ...
          <div className="fade-up">
            <div className="grid-5" style={{ marginBottom: 28 }}>
              {predLoad ? Array(5).fill(0).map((_, i) => <Skeleton key={i} h={110} />) : (
                <>
                  <MetricCard label={t.apr2026} value={`${nisTL.toFixed(1)} TL/kg`} />
                  <MetricCard label={t.dec2026} value={`${aralikTL.toFixed(1)} TL/kg`} delta={`${(aralikTL - nisTL) > 0 ? '+' : ''}${(aralikTL - nisTL).toFixed(1)} TL`} deltaNeg={aralikTL < nisTL} />
                  <MetricCard label={t.avg2026} value={`${ortTL.toFixed(1)} TL/kg`} />
                  <MetricCard label={t.tmoForecast} value={tmo ? `${tmo.pred_2026.toFixed(1)} TL/kg` : '…'} delta={tmo ? `%${tmo.buyume_pct} ${t.expectedIncrease}` : ''} />
                  <MetricCard label={t.marginOfError} value={`%${pred?.model_info?.test_mape?.toFixed(1) ?? '5.1'}`} delta="MAPE (Test)" />
                </>
              )}
            </div>
                ...
                <Plot
                  data={[
                    ...
                    ...(showCI ? [{
                      ...
                      name: t.ci90, hoverinfo: 'skip' as const,
                    }] : []),
                    {
                      x: predDates,
                      ...
                      text: preds.map((p: MonthlyPrediction) => `${translateMonth(p.ay_adi)}: ${p.tl.toFixed(1)} TL<br>USD: $${p.nominal_usd.toFixed(2)}`),
                  ...
                  layout={{
                    ...
                    annotations: [{ x: '2026-09-07', y: 1, yref: 'paper', text: t.harvest, showarrow: false, font: { color: 'rgba(255,152,0,0.7)', size: 11 } }],
                  }}
            ...
            {/* Tahmin Tablosu */}
            <div className="card">
              <div className="section-title">{t.tableTitle}</div>
              {predLoad ? <Skeleton h={300} /> : (
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
                  <thead>
                    <tr style={{ borderBottom: '1px solid var(--border)' }}>
                      {[t.thMonth, t.thUsdTry, t.thNominalUsd, t.thForecastTl, t.thLowCi, t.thHighCi].map(h => (
                  ...
                  <tbody>
                    {preds.map((p: MonthlyPrediction) => (
                      <tr key={p.ay} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                        <td style={{ padding: '10px 12px', fontWeight: 600 }}>{translateMonth(p.ay_adi)}</td>
        ...
        {tab === 'model' && (
          <div className="fade-up">
            <div className="grid-4" style={{ marginBottom: 28 }}>
              <MetricCard label={t.marginOfError} value={`%${pred?.model_info?.test_mape?.toFixed(2) ?? scores?.['Weighted Ensemble']?.MAPE?.toFixed(2) ?? '5.06'}`} delta="Delta Modeling" />
              ...
              <MetricCard label={t.causalEffectAte} value={causal ? `+${causal.average_treatment_effect} TL` : '…'} delta={t.causalEffectDesc} />
            </div>

            {scores && (
              <div className="card" style={{ marginBottom: 24 }}>
                <div className="section-title">{t.modelComparison}</div>
                <Plot
                  data={[{
            ...
            {causal && (
              <div className="card glass" style={{ marginBottom: 24 }}>
                <div className="section-title">{t.doubleMlEffect}</div>
            ...
            {shap && (
              <div className="card">
                <div className="section-title">{t.modelFactors}</div>
                <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: 16 }}>{t.modelFactorsDesc}</p>
                <Plot
                  data={[{
                  ...
                  layout={{
                    ...
                    xaxis: { gridcolor: 'rgba(255,255,255,0.06)', zeroline: false, title: { text: t.importanceScore } },
                    yaxis: { gridcolor: 'rgba(255,255,255,0.06)', zeroline: false },
                  }}
                  config={{ displayModeBar: false, responsive: true }}
                  style={{ width: '100%' }}
        ...
        {tab === 'whatif' && (
          <div className="fade-up">
            <div className="grid-3" style={{ marginBottom: 28 }}>
              <div className="card">
                <div className="section-title">{t.forexFinance}</div>
                <label style={{ display: 'block', marginBottom: 8, fontSize: '0.82rem', color: 'var(--text-secondary)' }}>{t.usdRate}: <strong style={{ color: 'var(--accent)' }}>{wiKur}</strong></label>
                ...
                <label style={{ display: 'block', marginTop: 16, marginBottom: 8, fontSize: '0.82rem', color: 'var(--text-secondary)' }}>{t.brentOil}: <strong style={{ color: 'var(--accent)' }}>${wiPetrol}</strong></label>
              ...
              <div className="card">
                <div className="section-title">{t.harvestYield}</div>
                <label style={{ display: 'block', marginBottom: 8, fontSize: '0.82rem', color: 'var(--text-secondary)' }}>{t.change}: <strong style={{ color: wiRekolte < 0 ? 'var(--accent-red)' : 'var(--accent-green)' }}>{wiRekolte > 0 ? '+' : ''}{wiRekolte}%</strong></label>
                ...
                <div style={{ marginTop: 12, fontSize: '0.78rem', color: 'var(--text-muted)' }}>
                  {wiRekolte < -20 ? t.shortage : wiRekolte > 20 ? t.bountiful : t.normalSeason}
                </div>
              </div>
              <div className="card">
                <div className="section-title">{t.scenarioResult}</div>
                {wi ? (
                  <>
                    <div style={{ fontSize: '2.2rem', fontWeight: 800, color: 'var(--accent-light)', marginBottom: 8 }}>{wi.whatif_tl.toFixed(1)} TL</div>
                    <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: 4 }}>{t.baseScenario}: {wi.baz_tl.toFixed(1)} TL</div>
